...
int main()
{
	...
	srand(time(0)); 
	array=rand()%3+1;
	if ( (fmora = fopen("mora.txt", "w") ) == NULL )
    	{
		printf("Can't create mora file . \n ");
		exit(1);
    	}
	fprintf(fmora,"%d\n",array);
    fclose(fmora);
	...
	else
	{
        ...
        switch(array)
        {
            case 1:
            {
                Write_Hand_Angle(0x01,1000,1000,1000,1000,1000,1000);
                break;
            }
            case 2:
            {
                Write_Hand_Angle(0x01,0,0,1000,1000,0,1000);
                break;
            }
            case 3:
            {
        ...
        //write(fd,tUartData.m_send_array,sizeof(*tUartData.m_send_array));
